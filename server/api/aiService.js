...
export const generateRecipeWithGemini = async (ingredientsArr, cuisine, meal, dietary, servings, cooking, time, flavor, skill, extraInc, name, email) => {
    ...
    let SYSTEM_PROMPT = `You are an assistant that receives a list of ingredients that a user has and suggests a recipe they could make. Use only the provided ingredients. Do not add any extra ingredients beyond what the user has mentioned. Respond **only** with valid JavaScript code, assigning the result to \`aiRecipe\` with **exact property names**: \`recipeName\`, \`ingredients\`, \`description\`, \`equipment\`, \`prepTime\`, \`cookTime\`, \`totalTime\`, \`difficulty\`, \`servings\`, \`cuisine\`, \`dietary\`, and \`instructions\`. No explanations, no formatting, no extra text. Example response: \`const aiRecipe = { recipeName: "Recipe Name", ingredients: ["Ingredient1", "Ingredient2"], description: "Short description.", equipment: ["Equipment1", "Equipment2"], prepTime: "15 minutes", cookTime: "30 minutes", totalTime: "45 minutes", difficulty: "Easy", servings: 2, cuisine: "Italian", dietary: ["Vegan", "Gluten-Free"], instructions: ["Step 1: Do this.", "Step 2: Do that."] };\` `;

    if (cuisine || meal || dietary || servings || cooking || time || flavor || skill || extraInc) {
